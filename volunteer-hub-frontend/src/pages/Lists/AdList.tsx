...
const AdList = ({ getAdListAction }: any) => {
  ...
  const createData = ({ id, title, summary, skills, location, isOpen }: any) => {
    const status = isOpen
      ? "Open for applications!"
      : "Sorry the ad is no longer open for application :c";
    return {id, title, summary, skills: skills, location, status: status };
  };

  const adList = useSelector((state: any) => state.ads.adList);
  const [adListRows, setAdListRows] = useState(
    adList.map((x) => createData(x))
  );
  ...
  return (
    <>
      <ListTemplate entityName={"ad"} rows={adListRows} fields={fields} avatarName={avatarName} />
    </>
  );
};
